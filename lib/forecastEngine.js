...
export function processData(rawRows, detectedCols, rainfallMap, scenarios, baselineWeek, modelType = 'estate', estateRainfall = {}) {
    ...
    scenarioResults = scenarios.map(scenarioMm => {
        ...
        baselineRows.forEach(row => {
            ...
            let dAvg = 0;
            if (estModel && estModel.fit.method === 'fit') {
                dAvg = estModel.fit.a + estModel.fit.b * scenarioMm;
            } else {
                // fallback to global average if estate lacks data
            ...
            // Accumulate dAvg for tracking
            if (validPoints === 1 || (estModel && estModel.fit.method === 'fit')) {
                // Just for display logic: rough average of deltas applied
                // Since this runs per piezometer, this average is heavily weighted by piezometer counts per estate
                totalDavg += dAvg; countDavg++;
            }
        });
