...
export function processData(rawRows, detectedCols, rainfallMap, scenarios, baselineWeek, modelType = 'estate', estateRainfall = {}) {
    ...
    // Collect ALL fitted models (regardless of R²) to get a data-driven natural drying rate
    const allFittedModels = estateModels.filter(m => m.fit.method === 'fit');
    // Only trust high-R² models for the primary slope
    const validModels = estateModels.filter(m => m.fit.method === 'fit' && m.fit.r2 >= R2_THRESHOLD);

    // Fallback intercept: use average from ALL fitted estates so CH=0 reflects natural drying,
    // or a physical default (+1.5 cm/week natural ground water recession) if no data available.
    const dataFallbackA = allFittedModels.length > 0 ? mean(allFittedModels.map(m => m.fit.a)) : 1.5;
    const dataFallbackB = allFittedModels.length > 0 ? mean(allFittedModels.map(m => m.fit.b)) : -0.05;

    fit = {
        a: validModels.length > 0 ? mean(validModels.map(m => m.fit.a)) : dataFallbackA,
        b: validModels.length > 0 ? mean(validModels.map(m => m.fit.b)) : dataFallbackB,
    ...
    scenarioResults = scenarios.map(scenarioMm => {
        ...
        baselineRows.forEach(row => {
            ...
            dAvg = estA + estB * scenarioMm;

            // Relative sanity check: more rain should NEVER make things drier than zero rain.
            // dAvg at CH=X should never exceed dAvg at CH=0 (which equals `estA`).
            // Since b < 0, this is naturally satisfied, but we clamp as an extra safeguard.
            const deltaAtZeroRain = estA;
            if (scenarioMm > 0 && dAvg > deltaAtZeroRain) {
                dAvg = deltaAtZeroRain;
            }
